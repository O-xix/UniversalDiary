...
public class EntryEditActivity extends AppCompatActivity {
    ...
    //need to edit, so that it grabs the event and rewrites over it.
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_entry_create);
        initWidgets();
        time = DailyViewActivity.editEntryTime;
        entryDateTV.setText("Date: " + CalendarUtils.formattedDate(DailyViewActivity.editEntryDate));
    ...
    public void saveEntryAction(View view) {
        ...
        Entry revisedEntry = new Entry(entryName, entryText, "", 0, DailyViewActivity.editEntryDate, time);
        Entry.entriesList.set(DailyViewActivity.editEntryInArray, revisedEntry);
        finish();
    }
    ...
    public void publishEntryAction(View view) {
        ...
        Entry publishedNewEntry = new Entry(entryName, entryPublishText, "", 0, CalendarUtils.selectedDate, time);
        Entry.entriesList.add(newEntry);
        Entry.publicEntriesList.add(publishedNewEntry);
        finish();
    }
